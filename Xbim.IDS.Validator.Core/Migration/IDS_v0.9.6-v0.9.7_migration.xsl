...
<xsl:stylesheet
  ...
  <!-- IDS-0.9.7: AW move specification's min/maxOccurs down to Applicability -->
  <xsl:template match="ids:specification/@minOccurs" />
  <xsl:template match="ids:specification/@maxOccurs" />
  <xsl:template match="ids:applicability">
    <xsl:copy>
  ...
  <xsl:template match="@xsi:schemaLocation">
    <xsl:attribute name="xsi:schemaLocation">http://standards.buildingsmart.org/IDS http://standards.buildingsmart.org/IDS/0.9.7/ids.xsd</xsl:attribute>
  </xsl:template>
